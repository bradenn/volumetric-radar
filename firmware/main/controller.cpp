...
class pulseData {
public:
    pulseData(uint32_t prf, uint32_t pulse) : prf(prf), pulse(pulse) {
...
static int tickCap = 1000;

static bool pulse_callback(gptimer_handle_t timer, const gptimer_alarm_event_data_t *edata, void *user_ctx) {
    auto *meta = (pulseData *)(user_ctx);
    pulseData pd = *meta;
    ticks = (ticks + 1) % pd.prf;
    if (ticks == 0) {
        gpio_set_level(GPIO_NUM_9, 1);
    } else if (ticks == pd.pulse) {
...
Controller::Controller(Adc *adc, uint32_t prf, uint32_t pulse) {
    ...
    gptimer_alarm_config_t alarm_on = {
            .alarm_count = 10, // period = 1s @resolution 1MHz
    ...
    gptimer_event_callbacks_t alarm_on_callback = {
            .on_alarm = pulse_callback, // register user callback
    };
    pd = new pulseData(prf/10, pulse/10);
    ESP_ERROR_CHECK(gptimer_register_event_callbacks(pulse_timer, &alarm_on_callback, pd));
